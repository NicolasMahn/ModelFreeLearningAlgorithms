...
def monte_carlo_generic(env, function, episodes, gamma, epsilon, alpha,
                        epsilon_decay, updates, prev_state):
    ...
    for episode in range(episodes + 1):
        ...
        # Update V values
        v = function(v, return_, n, gamma, alpha)
...
def monte_carlo(env, episodes=500, gamma=0.9, epsilon=0.4, epsilon_decay=0.99, updates=False, prev_state=True):
    function = monte_carlo_function
    return monte_carlo_generic(env, function, episodes, gamma, epsilon, None, epsilon_decay, updates, prev_state)


def monte_carlo_function(v, return_, n, gamma, alpha):

    return_sum = 0
    for i in range(len(return_)-1, -1, -1):
        state = list(return_)[i]
        return_sum = return_[state] + gamma * return_sum

        v[state] = v[state] + (1 / n[state]) * (return_sum - v[state])
    return v


def monte_carlo_alpha(env, episodes=500, gamma=0.9, epsilon=0.4, alpha=0.01, epsilon_decay=0.99, updates=False,
                      prev_state=True):
    function = monte_carlo_alpha_function
    return monte_carlo_generic(env, function, episodes, gamma, epsilon, alpha, epsilon_decay, updates, prev_state)


def monte_carlo_alpha_function(v, return_, n, gamma, alpha):
    return_sum = 0
    for i in range(len(return_)-1, -1, -1):
        state = list(return_)[i]
        return_sum = return_[state] + gamma * return_sum

        v[state] = v[state] + alpha * (return_sum - v[state])
    return v
