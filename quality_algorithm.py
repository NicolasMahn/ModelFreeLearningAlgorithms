...
def get_v_from_q(env, q_table):
    ...
    for state in env.all_possible_states:
        ...
        q_table[env.state_to_int(state), :].max()
...
def generic_quality_algorithm(env, function, episodes, gamma, epsilon, alpha, epsilon_decay, updates):
    ...
    for episode in range(episodes + 1):
        ...
        while not env.done(state):
            ...
            # Update Q value
            q_table[int_state, int_action] = function(env, q_table, reward, state, action, next_state, prev_states,
                                                      gamma, alpha, epsilon)
...
def q_learning(env, episodes=500, gamma=0.9, epsilon=0.4, alpha=0.01, epsilon_decay=0.99, updates=False):
...
def sarsa(env, episodes=500, gamma=0.9, epsilon=0.4, alpha=0.01, epsilon_decay=0.99, updates=False):
